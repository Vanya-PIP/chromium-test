...
inline constexpr auto kIgnoredActions = std::to_array({
    "MobileOmniboxUse",
...
inline constexpr auto kRethrownActions = std::to_array({
    "MobileTabSwitched",
});

// A list of actions that indicate a new task has been started.
inline constexpr auto kNewTaskActions = std::to_array({
...
// Min and max values for the buckets in the duration histograms.
constexpr base::TimeDelta kDurationHistogramMin = base::Minutes(5);
constexpr base::TimeDelta kDurationHistogramMax = base::Hours(48);

// Number of buckets in the duration histograms.
constexpr int kDurationHistogramBucketCount = 50;
    ...
    switch (action_type) {
      case NEW_TASK:
        UMA_HISTOGRAM_CUSTOM_COUNTS(
            ...
            background_duration_.InMinutes(), kDurationHistogramMin.InMinutes(),
            kDurationHistogramMax.InMinutes(), kDurationHistogramBucketCount);
        break;
      case CONTINUATION:
        UMA_HISTOGRAM_CUSTOM_COUNTS(
