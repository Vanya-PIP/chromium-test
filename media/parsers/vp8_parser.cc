...
bool Vp8Parser::ParseSegmentationHeader(bool keyframe) {
  ...
  if (keyframe)
    UNSAFE_TODO(memset(shdr, 0, sizeof(*shdr)));
...
bool Vp8Parser::ParseLoopFilterHeader(bool keyframe) {
  ...
  if (keyframe)
    UNSAFE_TODO(memset(lfhdr, 0, sizeof(*lfhdr)));
...
bool Vp8Parser::ParseQuantizationHeader(Vp8QuantizationHeader* qhdr) {
  ...
  UNSAFE_TODO(memset(qhdr, 0, sizeof(*qhdr)));
...
void Vp8Parser::ResetProbs() {
  ...
  UNSAFE_TODO(memcpy(curr_entropy_hdr_.coeff_probs, kDefaultCoeffProbs,
  ...
  UNSAFE_TODO(memcpy(curr_entropy_hdr_.mv_probs, kDefaultMVProbs,
  ...
  UNSAFE_TODO(memcpy(curr_entropy_hdr_.y_mode_probs, kDefaultYModeProbs,
  ...
  UNSAFE_TODO(memcpy(curr_entropy_hdr_.uv_mode_probs, kDefaultUVModeProbs,
...
bool Vp8Parser::ParseTokenProbs(Vp8EntropyHeader* ehdr,
  ...
  if (update_curr_probs) {
    UNSAFE_TODO(memcpy(curr_entropy_hdr_.coeff_probs, ehdr->coeff_probs,
                       sizeof(curr_entropy_hdr_.coeff_probs)));
  }
...
bool Vp8Parser::ParseIntraProbs(Vp8EntropyHeader* ehdr,
  ...
  if (keyframe) {
    static_assert(
        ...
        "y_probs_arrays_must_be_of_correct_size");
    UNSAFE_TODO(memcpy(ehdr->y_mode_probs, kKeyframeYModeProbs,
                       sizeof(ehdr->y_mode_probs)));

    static_assert(
        sizeof(ehdr->uv_mode_probs) == sizeof(kKeyframeUVModeProbs),
        "uv_probs_arrays_must_be_of_correct_size");
    UNSAFE_TODO(memcpy(ehdr->uv_mode_probs, kKeyframeUVModeProbs,
    ...
    if (intra_16x16_prob_update_flag) {
      ...
      if (update_curr_probs) {
        UNSAFE_TODO(memcpy(curr_entropy_hdr_.y_mode_probs, ehdr->y_mode_probs,
                           sizeof(curr_entropy_hdr_.y_mode_probs)));
      }
    }
    ...
    if (intra_chroma_prob_update_flag) {
      ...
      if (update_curr_probs) {
        UNSAFE_TODO(memcpy(curr_entropy_hdr_.uv_mode_probs, ehdr->uv_mode_probs,
                           sizeof(curr_entropy_hdr_.uv_mode_probs)));
      }
    }
  }
...
bool Vp8Parser::ParseMVProbs(Vp8EntropyHeader* ehdr, bool update_curr_probs) {
  ...
  if (update_curr_probs) {
    UNSAFE_TODO(memcpy(curr_entropy_hdr_.mv_probs, ehdr->mv_probs,
                       sizeof(curr_entropy_hdr_.mv_probs)));
  }
...
bool Vp8Parser::ParsePartitions(Vp8FrameHeader* fhdr) {
  ...
  const uint8_t* ptr = UNSAFE_TODO(
      fhdr->data + base::checked_cast<size_t>(fhdr->first_part_offset) +
      fhdr->first_part_size);

  // Read sizes from the stream (if present).
  for (size_t i = 0; i < fhdr->num_of_dct_partitions - 1; ++i) {
    fhdr->dct_partition_sizes[i] =
        (UNSAFE_TODO(ptr[2]) << 16) | (UNSAFE_TODO(ptr[1]) << 8) | ptr[0];
