...
constexpr auto kSigningKey1 = std::to_array<uint8_t>({
...
constexpr auto kSigningKey1Signature = std::to_array<uint8_t>({
...
constexpr auto kSigningKey2 = std::to_array<uint8_t>({
...
constexpr auto kSigningKey2Signature = std::to_array<uint8_t>({
...
std::string DMSigningKey::GetPublicKeyString() const {
  auto key = crypto::keypair::PrivateKey::FromPrivateKeyInfo(key_data_);

  return std::string(base::as_string_view(key->ToSubjectPublicKeyInfo()));
}

void DMSigningKey::SignData(const std::string& data,
                            std::string* signature) const {
  auto key = crypto::keypair::PrivateKey::FromPrivateKeyInfo(key_data_);
  auto sig = crypto::sign::Sign(crypto::sign::SignatureKind::RSA_PKCS1_SHA256,
                                *key, base::as_byte_span(data));
  *signature = base::as_string_view(sig);
}
